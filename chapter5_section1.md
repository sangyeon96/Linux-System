...
fstab은 file systems table의 약자이다.

파티션과 마운트 포인트, 구동 시에 어떤 파일 시스템을 어떤 순서로 마운트할 것인지에 대한 내용이 기술되어 있는 파일이 /etc/fstab이다.
...
* 마운트 옵션
* 덤프
* 파일 시스템 확인 순서 - fsck 순서 ex\) root fsck = 1

### 1.5 커맨드라인 입력 지원 라이브러리\(/etc/inputrc\)

/etc/inputrc는 readline 라이브러리에서 이용하는 설정 파일로, 키 바인딩\(단축키\)을 설정하는 것이다. 기본값일 때는 커서 이동이 에디터 Emacs의 단축키를 기준으로 가능하지만 vi의 단축키 기준으로도 변경할 수 있다.

![](/assets/readline control.png)

/etc/inputrc의 기술 방법에는 두 가지가 있다.
...
시스템 전체의 설정 파일을 /etc/inputrc에 배치한다. 사용자 저마다 개인 설정을 하는 경우 ~/.inputrc에 기술함으로써 개별 동작을 지정할 수 있다. 또한 단축키가 설정되어 있는지를 확인하려면 `bind -s`를 실행해보면 된다.
